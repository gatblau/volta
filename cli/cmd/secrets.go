...
func listSecrets(cmd *cobra.Command, args []string) error {
	...
	if outputJSON {
		...
		_, _ = fmt.Fprintln(w, "SECRET ID\tVERSION\tTYPE\tSIZE\tTAGS\tUPDATED")

		for _, secret := range secrets {
			_, _ = fmt.Fprintf(w, "%s\t%d\t%s\t%d\t%s\t%s\n",
...
func readSecretData() ([]byte, error) {
	...
	if secretFile != "" {
		...
		return os.ReadFile(absPath) // #nosec G304 -- path is cleaned and resolved to absolute
	}
