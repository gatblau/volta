...
func init() {
	...
	_ = tenantRotatePassphraseCmd.MarkFlagRequired("passphrase")
	...
	_ = tenantRotatePassphrasesCmd.MarkFlagRequired("passphrase")
...
func runTenantRotateKey(cmd *cobra.Command, args []string) error {
	...
	if !forceFlag {
		...
		if _, err := fmt.Scanln(&response); err != nil {
			// Ignore error - user may have entered empty input
			_ = err
		}
...
func displayBulkResults(operationType string, results []volta.BulkOperationResult) error {
	...
	if _, err := fmt.Fprintf(w, "TENANT\tSTATUS\tERROR\tTIMESTAMP\n"); err != nil {
		return err
	}
...
func showAuditSummaryAllTenants(since *time.Time) error {
	...
	w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
	if _, err := fmt.Fprintf(w, "TENANT\tTOTAL\tSUCCESS\tFAILED\tPASSPHRASE\tSECRETS\tKEYS\tLAST ACTIVITY\n"); err != nil {
		return err
	}
