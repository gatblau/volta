...
func listKeysForTenant(tenant string) error {
	...
	if _, err := fmt.Fprintf(w, "TENANT\tKEY ID\tSTATUS\tACTIVE\tCREATED\tVERSION\n"); err != nil {
		return err
	}

	for _, key := range keys {
		if _, err := fmt.Fprintf(w, "%s\t%s\t%s\t%t\t%s\t%d\n",
			tenant,
			key.KeyID,
			key.Status,
			key.Active,
...
func listKeysAllTenants() error {
	...
	w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
	if _, err := fmt.Fprintf(w, "TENANT\tKEY ID\tSTATUS\tCREATED\tVERSION\n"); err != nil {
		return err
	}
