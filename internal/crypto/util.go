...
func IsWeakKey(key []byte) bool {
	...
	// Detect very predictable sequential patterns
	if isSequentialPattern(key) {
		return true
	}

	// Detect repeating sub-patterns (e.g., ABABAB...)
...
func isSequentialPattern(key []byte) bool {
	if len(key) < 3 {
		return false
	}

	difference := (int(key[1]) - int(key[0]) + 256) % 256
	if difference != 1 && difference != 255 {
		return false
	}

	for i := 2; i < len(key); i++ {
		nextDifference := (int(key[i]) - int(key[i-1]) + 256) % 256
