...
func TestCustomDerivation(t *testing.T) {
	...
	if err := vault1.Close(); err != nil {
		t.Logf("Warning: failed to close vault1: %v", err)
	}
	...
	defer func() {
		if err := vault2.Close(); err != nil {
			t.Logf("Warning: failed to close vault2: %v", err)
		}
	}()
...
func TestKeyRotationCrypto(t *testing.T) {
	...
	defer func() {
		if err := cryptoManager.Close(); err != nil {
			t.Logf("Warning: failed to close crypto manager: %v", err)
		}
	}()
