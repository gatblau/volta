...
func testBulkOperationErrors(t *testing.T, manager VaultManagerService) {
	...
	results, _ := manager.RotateAllTenantPassphrases(tenantList, "new-test-passphrase", "test rotation")
	...
	for i := range results {
		switch results[i].TenantID {
		case "failing-tenant":
			failingResult = &results[i]
			if !results[i].Success {
				hasFailures = true
			}
		case "working-tenant-bulk":
	...
	if failingResult == nil {
		...
		switch {
		case failingResult.Success:
			t.Error("Expected failure for failing-tenant, but got success")
		case failingResult.Error == "":
			t.Error("Expected error message for failing-tenant")
		case !strings.Contains(failingResult.Error, "mock store creation failure"):
