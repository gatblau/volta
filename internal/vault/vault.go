...
func (v *Vault) initializeSecretsContainer() error {
	...
	// Check if secrets data is empty
...
func (v *Vault) encryptWithKeyEnclave(data []byte, enclave *memguard.Enclave) ([]byte, error) {
	...
	// Get the raw key bytes and copy them before destroying the buffer
	keyBytes := make([]byte, len(buffer.Bytes()))
...
func (v *Vault) withRetry(operation string, fn func() error) error {
	...
	for attempt := 0; attempt <= config.MaxRetries; attempt++ {
		...
		if concErr, ok := err.(interface{ IsConcurrencyError() bool }); ok && concErr.IsConcurrencyError() {
			...
			// Add jitter (±25%) derived from crypto/rand so the offset is
			// unpredictable and cannot be exploited through timing analysis.
			delay += cryptoJitter(delay, 0.25)
...
// cryptoJitter returns a signed random duration in [−fraction*base, +fraction*base]
// using crypto/rand so jitter values are unpredictable and timing analysis
// resistant.  Falls back to 0 if random bytes cannot be read.
func cryptoJitter(base time.Duration, fraction float64) time.Duration {
	var b [8]byte
	if _, err := rand.Read(b[:]); err != nil {
		return 0 // safe default: no jitter rather than a panic
	}
	// Interpret 8 random bytes as an unsigned 64-bit integer, then map to [-1, 1).
	randUint := binary.BigEndian.Uint64(b[:])
	// Convert to float64 in [0, 1), then shift to [-1, 1).
	f := float64(randUint)/float64(1<<64)*2 - 1
	return time.Duration(f * fraction * float64(base))
}
