...
func TestStoreAndLoadSecret(t *testing.T) {
	...
	defer func(vault *Vault) {
		err := vault.Close()
		if err != nil {
			t.Errorf("storeAndLoadSecret: error closing vault: %v", err)
		}
	}(vault)
...
func TestListSecretsEmpty(t *testing.T) {
	...
	defer func(vault *Vault) {
		err := vault.Close()
		if err != nil {
			t.Fatalf("Failed to close vault connection: %v", err)
		}
	}(vault)
...
func TestConcurrentSecretOperations(t *testing.T) {
	...
	defer func(vault *Vault) {
		err := vault.Close()
		if err != nil {
			t.Errorf("Failed to close vault connection: %v", err)
		}
	}(vault)
