...
func (v *Vault) Encrypt(plaintext []byte) (ciphertextWithKeyID string, err error) {
	...
	_ = v.audit.Log("encrypt_data", true, map[string]interface{}{
	...
	if err != nil {
		_ = v.audit.Log("encrypt_data", false, map[string]interface{}{
	...
	// Security (G115): verify length BEFORE casting to uint16 to prevent silent
	// integer overflow truncation.  A key ID of ≥ 65,536 bytes would wrap to a
	// smaller uint16 value, producing a corrupted header.
	keyIDBytes := []byte(v.currentKeyID)
	keyIDLen := len(keyIDBytes)
	if keyIDLen > math.MaxUint16 { // G115: explicit bounds check - len() is always non-negative
		_ = v.audit.Log("encrypt_data", false, map[string]interface{}{
			"error": "key ID too long",
		})
		return "", errors.New("key ID too long")
	}
	keyIDLength := uint16(keyIDLen)
...
func (v *Vault) Decrypt(base64CiphertextWithKeyID string) ([]byte, error) {
	...
	_ = v.audit.Log("decrypt_data", true, map[string]interface{}{
	...
	if err != nil {
		_ = v.audit.Log("decrypt_data", false, map[string]interface{}{
