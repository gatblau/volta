...
func (v *Vault) RotateKeyEncryptionKey(newPassphrase string, reason string) error {
	...
	if err := v.saveKeyMetadata(rotationMetadata); err != nil {
		...
		if rollbackErr := v.saveSaltWithRetry(oldSaltBytes); rollbackErr != nil {
			...
			if v.audit != nil {
				_ = v.audit.Log("critical_passphrase_rotation_rollback_failed", false, map[string]interface{}{
	...
	// Destroy old derivation key and salt - memguard handles cleanup automatically
	// when the references go out of scope
