...
func (s3s *S3Store) LoadSecretsData() (*VersionedData, error) {
	...
	object, err := s3s.client.GetObject(ctx, s3s.bucketName, objectName, minio.GetObjectOptions{})
	if err != nil {
		// Return os.ErrNotExist directly to ensure os.IsNotExist() works
	...
	if err != nil {
		...
		// This handles cases where GetObject returns an object but reading fails
		if s3s.isNotFoundError(err) || strings.Contains(err.Error(), "does not exist") {
			return nil, os.ErrNotExist
		}
