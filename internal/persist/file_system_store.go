...
func (fs *FileSystemStore) SaveMetadata(encryptedMetadata []byte, expectedVersion string) (string, error) {
	...
	// Security (G301): use DirPermissions (0700) — not world-readable 0755.
	if err := os.MkdirAll(fs.tenantPath, DirPermissions); err != nil {
...
func (fs *FileSystemStore) LoadMetadata() (*VersionedData, error) {
	...
	data, err := os.ReadFile(filepath.Clean(fs.vaultMeta)) // #nosec G304 -- path is a pre-built struct field
	if err != nil {
...
func (fs *FileSystemStore) LoadSalt() (*VersionedData, error) {
	...
	// Read salt data
	saltData, err := os.ReadFile(filepath.Clean(fs.vaultSalt)) // #nosec G304 -- path is a pre-built struct field
	if err != nil {
...
func (fs *FileSystemStore) LoadSecretsData() (*VersionedData, error) {
	...
	data, err := os.ReadFile(filepath.Clean(fs.secretsMeta)) // #nosec G304 -- path is a pre-built struct field
	if err != nil {
		if os.IsNotExist(err) {
			return nil, err
		}
...
func (fs *FileSystemStore) RestoreBackup(backupPath string) (*BackupContainer, error) {
	...
	// Canonicalise and check if file exists
	...
	data, err := os.ReadFile(fullPath) // #nosec G304 -- path is sanitised by filepath.Clean above
	if err != nil {
...
func (fs *FileSystemStore) DeleteBackup(backupID string) error {
	...
	for _, entry := range entries {
		...
		filePath := filepath.Clean(filepath.Join(fs.backupsDir, entry.Name()))
		debug.Print("DeleteBackup: Checking file: %s\n", entry.Name())

		// Read and parse the backup file
		data, err := os.ReadFile(filePath) // #nosec G304 -- path is constructed from a trusted directory + dir-entry name
		if err != nil {
...
func (fs *FileSystemStore) Close() error {
	if configData, err := os.ReadFile(filepath.Clean(fs.vaultConfig)); err == nil { // #nosec G304 -- path is a pre-built struct field
...
func (fs *FileSystemStore) getFileVersion(filePath string) (string, error) {
	data, err := os.ReadFile(filepath.Clean(filePath)) // #nosec G304 -- callers only pass pre-built struct paths
	if err != nil {
		if os.IsNotExist(err) {
...
// calculateFileVersion returns a hex-encoded SHA-256 digest of the supplied data.
// It is used as an optimistic-concurrency version token for stored files.
// Security (G401/G501): MD5 was replaced with SHA-256 to eliminate collision risk.
...
func readMetadata(filePath string) (map[string]string, error) {
	metadataPath := filepath.Clean(filePath + ".meta")
	metadataBytes, err := os.ReadFile(metadataPath) // #nosec G304 -- path is sanitised by filepath.Clean above
	if err != nil {
		return nil, err
	}
