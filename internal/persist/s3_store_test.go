...
// TestS3StoreLoadNonexistentSecretsReturnsOsErrNotExist tests that loading non-existent
// secrets data returns an error that is recognized by os.IsNotExist().
// This is a regression test for the bug where GetVault fails for new tenants.
func TestS3StoreLoadNonexistentSecretsReturnsOsErrNotExist(t *testing.T) {
	// Always clear the endpoint env var to ensure each test gets its own container
	// This prevents the issue where a stale endpoint from a previous test is reused
	defer func() {
		_ = os.Unsetenv("S3_MINIO_ENDPOINT")
	}()
	...
	if len(endpoint) == 0 {
		...
		_ = os.Setenv("S3_MINIO_ENDPOINT", fmt.Sprintf("http://localhost:%s", mappedPort.Port()))
	}

	t.Run("LoadSecretsDataNotFoundIsOsErrNotExist", func(t *testing.T) {
		runTestLoadSecretsDataNotFoundIsOsErrNotExist(t)
	})
}

func runTestLoadSecretsDataNotFoundIsOsErrNotExist(t *testing.T) {
	// Get configuration from environment or use defaults for testcontainers
	bucketName := os.Getenv("S3_BUCKET")
	if bucketName == "" {
		bucketName = "test-volta-store-os-err-not-exist"
	}
	...
	if endpointURL == "" {
		t.Fatal("S3_MINIO_ENDPOINT not set")
	}

	endpoint, useSSL := parseEndpoint(endpointURL)
	region := "us-east-1"
	keyPrefix := "test-os-err-not-exist/"

	// Create a NEW store with a unique tenant that has no secrets
	uniqueTenant := "nonexistent-tenant-test-" + fmt.Sprintf("%d", time.Now().UnixNano())
	...
	if err != nil {
		t.Fatalf("Failed to check secrets existence: %v", err)
	}
	if exists {
		t.Fatal("Secrets should not exist for a new tenant")
	}

	// Try to load secrets data - should return error
	_, err = store.LoadSecretsData()
	if err == nil {
		t.Fatal("Expected error when loading non-existent secrets data")
	}

	// KEY TEST: Verify the error is recognized by os.IsNotExist()
	// This is the core fix for the bug where GetVault fails for new tenants
	if !os.IsNotExist(err) {
		t.Errorf("Error should be recognized by os.IsNotExist(), but got: %v", err)
	}

	t.Logf("Successfully verified os.IsNotExist() returns true for error: %v", err)
}
